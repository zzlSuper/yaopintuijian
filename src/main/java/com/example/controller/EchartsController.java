...
public class EchartsController {
    ...
    Result<List<EchartsData>> getEchartsData(@PathVariable String modelName) {
        ...
        switch (modelName) {
			...
			case "zhinengInfo":
				List<ZhinengInfoVo> zhinengInfoList = zhinengInfoService.findAll();
				Map<String, Integer> zhinengInfoMap = new HashMap<>(2);
				for (ZhinengInfo zhinengInfo : zhinengInfoList) {
					Integer value = zhinengInfoMap.get(zhinengInfo.getName());
					if (value != null && value != 0) {
						zhinengInfoMap.put(zhinengInfo.getName(), value + 1);
					} else {
						zhinengInfoMap.put(zhinengInfo.getName(), 1);
					}
				}
				getPieData("智能判断", list, zhinengInfoMap);
				getBarData("智能判断", list, zhinengInfoMap);
    ...
    @GetMapping("/options")
		...
		optionMap3.put("value", "zhinengInfo");
		optionMap3.put("label", "智能判断信息");
